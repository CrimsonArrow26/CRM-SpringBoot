...
public class AccountService {
    ...
    public AccountService(AccountRepository accountRepository, OrganizationRepository organizationRepository,
    ...
    @Transactional(readOnly = true)
    public Page<AccountDto> getAccountsByOrganizationPaginated(Long orgId, String search, Pageable pageable) {
        Organization organization = organizationRepository.findById(orgId)
                .orElseThrow(() -> new RuntimeException("Organization not found"));

        org.springframework.data.domain.Page<Account> page;
        if (search != null && !search.isBlank()) {
            page = accountRepository.findByOrganizationWithSearch(organization, search, pageable);
        } else {
            page = accountRepository.findByOrganizationWithRelations(organization, pageable);
        }
